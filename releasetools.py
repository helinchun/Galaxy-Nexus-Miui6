...
def AddArgsForFormatSystem(info):
  edify = info.script
  for i in xrange(len(edify.script)):
    if "format(" in edify.script[i] and "/dev/block/platform/omap/omap_hsmmc.0/by-name/system" in edify.script[i]:
      edify.script[i] = 'format("ext4", "EMMC", "/dev/block/platform/omap/omap_hsmmc.0/by-name/system", "0", "/system");'
      return
...
def FullOTA_InstallRecovery(info):
  ...
  InstallImage("recovery.img", img_file, "/dev/block/platform/omap/omap_hsmmc.0/by-name/recovery", info)
...
def IncrementalOTA_InstallRecovery(info):
  try:
    ...
    if source_file != target_file:
      info.script.Print("Writing recovery")
      InstallImage("recovery.img", target_file, "/dev/block/platform/omap/omap_hsmmc.0/by-name/recovery", info)
    else:
